...
class LeNet(object):
    ...
    def __init__(self, lr = 0.1):
        ...
        self.pool1 = [2, 2]
        # 16 convolution kernal, each has 6 * 5 * 5 sizeJ
    ...
    def backward_prop(self, softmax_output, output_label):
        l8_delta             = 2*(output_label - softmax_output)/softmax_output.shape[0]
        l7_delta             = self.relu(self.l7, l8_delta, deriv=True)                   # (batch_sz, 10)
        #N>0; deriv_fc2 = N*2(y-y^)*derivRelu(z)*l6; z=Wx+bias=l7; l6=input_fc2
        #l6_delta = l7_delta * fc2
        l6_delta, self.fc2   = self.fully_connect(self.l6, self.fc2, l7_delta, deriv=True)  # (batch_sz, 200)
        l5_delta             = self.relu(self.l5, l6_delta, deriv=True)                # (batch_sz, 200)
        #N>0; deriv_fc1 = N*2(y-y^)*derivRelu(z)*l4; z=Wx+bias=l5; l4=input_fc1
    ...
    def fully_connect(self, input_data, fc, front_delta=None, deriv=False):
        ...
        if deriv == False:
            ...
            # front->back (propagate loss)
            back_delta = np.dot(2 * front_delta, fc.T).reshape(input_data.shape)
            # update weights
...
if __name__ == '__main__':
    ...
    test_imgs = fetch_MNIST.load_test_images()
    test_labs = fetch_MNIST.load_test_labels().astype(int)#10000
    ...
    lr = 0.01;     max_iter = 937; iter_mod = int(data_size/batch_sz)
    print(iter_mod)
    train_labs = convertToOneHot(train_labs)
    test_labs = convertToOneHot(test_labs)  # 10000, 10
    print(train_labs)
    my_CNN = LeNet(lr)
    ...
    softmax_output = my_CNN.forward_prop(test_imgs)
    correct_list = [int(np.argmax(softmax_output[i]) == np.argmax(test_labs[i])) for i in range(9999)]
    accuracy = float(np.array(correct_list).sum()) / 9999
